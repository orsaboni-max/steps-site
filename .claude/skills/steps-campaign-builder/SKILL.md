...
| לידים בטופס → Arbox | OUTCOME_LEADS + Instant Form | **הצינור החי שלנו (עודכן 9/6):** טופס 1542048374099477 → Zapier (Zap 367117682) → Arbox + הבוט. Lead Gen ToS כבר אושר, הטופס חי. |
| מודעוּת (קמפיין מותג/השקה) | OUTCOME_AWARENESS | רק אם יש סיבה אסטרטגית — אחרת לא לבזבז תקציב |

**ברירת מחדל ל-STEPS (עודכן 9/6): טופס לידים (OUTCOME_LEADS) → Zapier → Arbox — הצינור החי.** CTWA הוא חלופה תקפה. בחר לפי המטרה, לא אוטומטית.
