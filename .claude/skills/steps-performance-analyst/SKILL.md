...
> ⚙️ **מ-9/6/26 החוקים האלה רצים גם אוטומטית בבוט** — `app/triggers/action_engine.py` (steps-brain) מיישם את טבלת SCALE/HOLD/REFRESH/KILL שלמטה כל יום א' 08:05 ושולח לאור הצעות עם כפתורי אישור. לפני אבחון ידני: בדוק מה המנוע כבר המליץ (`/weekly` בטלגרם). אם משנים חוק כאן → לעדכן גם את action_engine.py.
> 💰 **שיפוט הכנסה/המרה = רישומי קופה בלבד** (`/reports/salesReport`: paid, lead_source, membership_type_name). "אימון היכרות" = מוצר ₪50 בקופה = שלב-משפך קשיח. **אסור** `/membership` למחירים (price=null) וסטטוסי לידים כעובדה (הזנה ידנית = אינדיקציה בלבד). המשפך המלא: `/funnel` בבוט.
